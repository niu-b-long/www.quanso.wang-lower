...
if (RUN_ACTION === 'affiliate_list') {
	...
	foreach ((array) $user as $u) {
      	$zong = dr('admin/stats.get_all', $u['uid']);
		$l = dr('admin/class.get_one', $u['classid']);
		$g = dr('admin/group.get_one', $u['groupid']);
		$p = dr('admin/ping.get_one', $u['pingid']);
		$deduction = unserialize($u['deduction']);
		$money = (0 < $u['money'] ? round($u['money'], 2) : '0');
		$daymoney = dr('admin/stats.get_time_day_uid_money',$u['uid'],date('Y-m-d',time()));
      	$lastdaymoney = dr('admin/stats.get_time_day_uid_money',$u['uid'],date('Y-m-d',time()-86400));
		...
		echo '</a></td>' . "\r\n" . '            <td>';
      	$money=0;
      	foreach((array) $zong as $zongy){
          $money =$zongy['sumpay'] + $money;
        }
		echo round($money,2);
		...
		echo $u['zhifu'];
		echo '</a></td>' . "\r\n" . '            <td>';
		echo round($money-$u['zhifu'],2);
		echo '</td>' . "\r\n" . '            <td>';
		echo round($daymoney['sumpay'],2);
		echo '</td>' . "\r\n" . '            <td>';
		echo round($lastdaymoney['sumpay'],2);
		echo '</td>' . "\r\n" . '            <td>';
      	$weekriqi = (int)date('w');
      	if($weekriqi == 0){
        	$lastweekmoney = 0;
        	$weekmoney = dr('admin/stats.get_wmoney_this',$u['uid']);
          	foreach($weekmoney as $v){
            	$lastweekmoney = $lastweekmoney + $v['sumpay'];
            }
        }else if($weekriqi <= 2){
          	$lastweekmoney = 0;
        	$weekmoney = dr('admin/stats.get_wmoney_last',$u['uid']);
          	foreach($weekmoney as $v){
            	$lastweekmoney = $lastweekmoney + $v['sumpay'];
            }
        }else{
		...
		echo ' </a></td>' . "\r\n" . '            <td><a href="#" class="editable_a e_ping"   uid="';
		echo $u['uid'];
		echo '" pingid="';
		echo $p['pingid'];
		echo '" >';
		echo $p['pingname'] ? $p['pingname'] : '未分组';
		echo ' </a></td>' . "\r\n" . '            <td><a href="#" class="editable_a e_class"   uid="';
		echo $u['uid'];
		echo '" classid="';
		echo $l['classid'];
		echo '" >';
		echo $l['classname'] ? $l['classname'] : '未分组';
		echo '</a></td>' . "\r\n" . '            <td class="status">';
		...
		echo '"  target="_blank">编辑</a> <a href="';
		echo '"#" id="uld_unlock" uid="' . $u['uid'];
		echo '"  target="_blank">激活</a> <a href="';
		echo '"#" id="uld_lock" uid="' . $u['uid'];
		echo '"  target="_blank">锁定</a> <a href="';
		echo '"#" id="uld_del" uid="' . $u['uid'];
		echo '"  target="_blank">删除</a> <a href="';
		...
		$moneyall = dr('admin/stats.get_allmoney');
  		$zhifuall = dr('admin/user.get_allzhifu');
  		$daymoneyall = dr('admin/stats.get_daymoney',date('Ymd',time()));
  		$lastdaymoneyall = dr('admin/stats.get_daymoney',date('Ymd',time()-86400));
	echo '        </tbody>' . "\r\n";
  	echo '<tr role="row">' . "\r\n" . '            <th class="table_checkbox sorting_disabled" role="columnheader" rowspan="1" colspan="1" style="width: 13px;" aria-label="">汇总</th>' . "\r\n" . '            <th><em></em></th>' . "\r\n" . '            <th></th>' . "\r\n" . '            <th>'.round($moneyall,2).'</th>' . "\r\n" . '            <th>'.$zhifuall.'</th>' . "\r\n" . '            <th>'.round($moneyall-$zhifuall,2).'</th>' . "\r\n" . '            <th>'.round($daymoneyall,2).'</th>' . "\r\n" . '            <th>'.round($lastdaymoneyall,2).'</th>' . "\r\n" . '            <th></th>' . "\r\n" . '            <th></th>' . "\r\n" . '            <th></th>' . "\r\n" . '            <th></th>' . "\r\n" . '            <th></th>' . "\r\n" . '            </tr>' . "\r\n" . '        </thead>' . "\r\n" . '        <tbody role="alert" aria-live="polite" aria-relevant="all">' . "\r\n" . '          ';
  	echo   '      </table>'."\r\n" . '    </div>' . "\r\n" . '    ';
}
...
echo '    ';
if (RUN_ACTION === 'service_list') {
	echo '    <table id="dt_inbox" class="dataTable">' . "\r\n" . '      <thead>' . "\r\n" . '        <tr role="row">' . "\r\n" . '          <th class="table_checkbox sorting_disabled" role="columnheader" rowspan="1" colspan="1" style="width: 13px;" aria-label=""><input type="checkbox" name="select_id" id="select_id" /></th>' . "\r\n" . '          <th><em>ID</em></th>' . "\r\n" . '          <th>客服名称</th>' . "\r\n" . '          <th>在线站长</th>' . "\r\n" . '          <th>今日消耗</th>' . "\r\n" . '          <th>昨日消耗</th>' . "\r\n" . '          <th title="有他名下会员当月产生的佣金费用">上月消耗</th>' . "\r\n" . '          <th>本月消耗</th>' . "\r\n" . '           <th>完成度</th>' . "\r\n" . '          <th>状态</th>' . "\r\n" . '          <th>操作</th>' . "\r\n" . '        </tr>' . "\r\n" . '      </thead>' . "\r\n" . '      <tbody role="alert" aria-live="polite" aria-relevant="all">' . "\r\n" . '        ';
	$dayalls = 0;$monthalls = 0;$lastdayalls = 0;$lastmonthalls = 0;
	foreach ((array) $user as $u) {
		echo '        <tr class="unread odd">' . "\r\n" . '          <td><input type="checkbox" name="uid" id="uid_';
		echo $u['uid'];
		echo '" value="';
		...
		echo '个</a></td>' . "\r\n" . '          <td>';
		$days = dr('admin/stats.get_performance', $u['uid'], $get_timerange['day'], 'service');
      	$day = abs($days['pay']);
      	$day!=''?$day:0;
      	echo round($day,2);
      	$dayalls = $dayalls+$day;
      	echo '            元</td>' . "\r\n" . '          <td>';
		$yesterdays = dr('admin/stats.get_performance', $u['uid'], $get_timerange['yesterday'], 'service');
      	$yesterday = abs($yesterdays['pay']);
      	$yesterday!=''?$yesterday:0;
      	echo round($yesterday,2);
      	$lastdayalls = $lastdayalls + $yesterday;
      	echo '            元</td>' . "\r\n" . '          <td>';
		$lastmonths = dr('admin/stats.get_performance', $u['uid'], $get_timerange['lastmonth'], 'service');
      	$lastmonth = abs($lastmonths['pay']);
      	$lastmonth!=''?$lastmonth:0;
      	echo round($lastmonth,2);
      	$lastmonthalls = $lastmonthalls + $lastmonth;
      	echo '            元</td>' . "\r\n" . '          <td>';
		$thismonths = dr('admin/stats.get_performance', $u['uid'], $get_timerange['thismonth'], 'service');
      	$thismonth = abs($thismonths['pay']);
      	$thismonth!=''?$thismonth:0;
      	echo round($thismonth,2);
      	$monthalls = $monthalls + $thismonth;
      	echo '            元</td>' . "\r\n" . '          <td>';
		if($lastmonth=='')
        {
        	echo '0';
        ...
        {
        	$chushu = $thismonth!=''?$thismonth:0;
          	echo (round($chushu/$lastmonth,2))*100;
        }
		echo '            %</td>' . "\r\n" . '          <td class="status">';
		...
		echo '<tr>' . "\r\n" . '          <th>汇总</th>' . "\r\n" . '          <th></th>' . "\r\n" . '          <th></th>' . "\r\n" . '          <th></th>' . "\r\n" . '          <th>'.round($dayalls,2).'元</th>' . "\r\n" . '          <th>'.round($lastdayalls,2).'元</th>' . "\r\n" . '          <th>'.round($lastmonthalls,2).'元</th>' . "\r\n" . '          <th>'.round($monthalls,2).'元</th>' . "\r\n" . '           <th></th>' . "\r\n" . '          <th></th>' . "\r\n" . '          <th></th>' . "\r\n" . '        </tr>';
	echo '      </tbody>' . "\r\n" . '    </table>' . "\r\n" . '    ';
}
...
echo '",' . "\r\n\t" . 'e_ping:"';
echo url('admin/user.update_ping');
echo '",' . "\r\n\t" . 'e_class:"';
echo url('admin/user.update_class');
echo '",' . "\r\n\t" . 'del:"';
...
echo ']}];' . "\t\r\n\r\n" . 'var e_ping_columns' . "\t" . '= [' . "\r\n\t\t" . ' ' . "\t" . ' {type:"hidden", name:"uid"},' . "\r\n" . ' ' . "\t\t" . '     {type:"select", name:"pingid",option:[' . "\r\n\t\t\t" . '   ';
foreach ((array) $ping as $p) {
	$gp[] = ' {value: ' . $p['pingid'] . ', text: \'' . $p['pingname'] . '\'}';
}

echo implode(',', (array) $gp);
echo ']}];' . "\t\r\n\r\n" . 'var e_class_columns' . "\t" . '= [' . "\r\n\t\t" . ' ' . "\t" . ' {type:"hidden", name:"uid"},' . "\r\n" . ' ' . "\t\t" . '     {type:"select", name:"classid",option:[' . "\r\n\t\t\t" . '   ';
foreach ((array) $class as $l) {
	$lx[] = ' {value: ' . $l['classid'] . ', text: \'' . $l['classname'] . '\'}';
}

echo implode(',', (array) $lx);
echo '              ]},        ' . "\r\n" . '             ];' . "\r\n" . ' </script> ' . "\r\n" . '<script type="text/javascript" src="';
echo SRC_TPL_DIR;
