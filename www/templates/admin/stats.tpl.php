...
echo '%</th>' . "\r\n" . '            <th>￥';
if ((RUN_ACTION === 'plan_list') || (RUN_ACTION === 'ads_list')) {
	echo 0 < $sum_stats['sumadvpay'] ? round($sum_stats['sumadvpay'], 2) : 0;
}
else {
	echo 0 < $sum_stats['sumpay'] ? round($sum_stats['sumpay'], 2) : 0;
}
...
foreach ((array) $stats as $s ) {
	$p = dr('admin/plan.get_one', $s['planid']);
 	...
 	$zhan = dr('admin/user.get_one', $s['uid']);
	...
	switch (RUN_ACTION) {
	case RUN_ACTION:
		echo '<a href="' . url('admin/plan.get_list?searchtype=planid&search=' . $p['planid']) . '">' . $zhan['username'] . '</a>';
	...
	echo '%</td>' . "\r\n" . '            <td>￥';
	if ((RUN_ACTION === 'plan_list') || (RUN_ACTION === 'ads_list')) {
		echo 0 < $s['sumadvpay'] ? round($s['sumadvpay'], 2) : 0;
	}
	else {
		echo 0 < $s['sumpay'] ? round($s['sumpay'], 2) : 0;
	}
