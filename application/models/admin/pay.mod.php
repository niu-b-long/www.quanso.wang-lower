...
class pay_mod extends app_models
{
	...
	public function get_list()
	{
		...
		if ($status == 1) {
			...
			$this->order_by('pay.status,pay.addtime');
		}
	...
	public function post_payment($id)
	{
		...
		if ($get_pay) {
			...
			$this->set($data);
			$data = $this->update();
          	dr('admin/user.update_daymoney',$get_pay['uid'],$get_pay['money']);
	...
	public function del($id)
	{
  	...
  	public function add_post($uid,$money,$clearingtype)
	{
		$data=array('uid'=>$uid,'pay'=>$money,'money'=>$money,'status'=>1,'addtime'=> DATETIMES,'paytime' => DATETIMES,'clearingtype'=>$clearingtype);
		$this->set($data);
		$this->insert();
	}
  
  	public function add_post_wmoney($uid,$money,$group)
    {
    	$data=array('uid'=>$uid,'pay'=>$money,'money'=>$money,'status'=>0,'addtime'=> date('Y-m-d', strtotime('last week')),'clearingtype'=>$group);
      	$this->set($data);
		$this->insert();
    }
  
  	public function get_wmoney($uid,$addtime)
    {
    	$where = array('uid' => $uid,'addtime' => $addtime);
      	$this->where($where);
      	$data = $this->get();
      	return $data;
    }
