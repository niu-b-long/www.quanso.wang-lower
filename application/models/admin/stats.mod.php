...
class stats_mod extends app_models
{
	...
	public function get_time_day_uid($uid, $time)
	{
		$this->select('sum(sumpay) as sumpay');
		$time = explode('_',$time);
		$this->where(array('uid' => $uid,'day' => $time[1]));
      	//$this->where('day',$time[1]);
		$this->group_by('day');
		$data = $this->find_one();
		return $data;
	}
  
  	public function get_time_day_uid_money($uid,$time)
    {
    	$this->where('uid',$uid);
      	$this->where('day',$time);
		//$this->group_by('day');
  	...
  	public function get_time_month_uid($uid, $time)
	{
      	$this->select('sum(sumpay) as sumpay');
		$time = explode('_',$time);
		$this->where(array('uid' => (int) $uid, 'day >=' => date('Ymd', strtotime($time[0])), 'day <=' => date('Ymd', strtotime($time[1]))));
		//$this->group_by('day');
		$data = $this->find_one();
		return $data;
	}
	...
	public function get_day_performance($uid, $timerange, $type)
	{
		...
		return $data;
  	  	...
  	  	public function get_wmoney_last($uid)
    {
    	$this->where(array('uid' => $uid, 'day >=' => date('Y-m-d', strtotime('last week')), 'day <=' => date('Y-m-d', strtotime('last week') + 518400)));
      	$data = $this->get();
      	return $data;
    }
  
  	  	public function get_wmoney_this($uid)
    {
    	$this->where(array('uid' => $uid, 'day >=' => date('Y-m-d', strtotime('last week') + 604800)));
      	$data = $this->get();
      	return $data;
    }
  		public function get_allmoney()
        {
  		...
  		public function get_daymoney($date)
        {
        	$this->select('sum(sumpay) as sumpay');
          	$this->where(array('day' => $date));
          	$data = $this->find_one();
          	return $data['sumpay'];
        }
}
