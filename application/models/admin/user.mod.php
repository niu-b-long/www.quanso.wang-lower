...
class user_mod extends app_models
{
	...
	{
      	$oldmoney=$this->get_one($uid);
		$where = array('uid' => (int) $uid);
		$data = array('zhifu' => $money+$oldmoney['zhifu']);
		$this->where($where);
		$this->set($data);
		$data = $this->update();
	}
	...
	public function update_ping($uid, $pingid)
	{
		$where = array('uid' => (int) $uid);
		$data = array('pingid' => $pingid);
		$this->where($where);
		$this->set($data);
		$data = $this->update();
	...
	public function update_class($uid, $classid)
	{
		$where = array('uid' => (int) $uid);
		$data = array('classid' => (int) $classid);
		$this->where($where);
		$this->set($data);
		$data = $this->update();
	...
	public function get_commercial_user()
	{
  	...
  	public function get_allzhifu()
    {
    	$this->select('sum(zhifu) as zhifu');
      	$data = $this->find_one();
      	return $data['zhifu'];
    }
}
