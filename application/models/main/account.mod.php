...
class account_mod extends app_models
{
	...
	{
		$where = array('uid' => (int) $uid);
		$data = array('mobile' => post('mobile'), 'qq' => post('qq'), 'email' => post('email'), 'tel' => post('tel'), 'idcard' => post('idcard'), 'contact' => post('contact'), 'bankbranch' => post('bankbranch'), 'bankaccount' => post('bankaccount'), 'accountname' => post('accountname'));
		$this->where($where);
		$this->set($data);
		$data = $this->update();
	}
