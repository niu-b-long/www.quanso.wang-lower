...
class demo_ctl extends appController
{
	...
	{	
		//$type = (int) get('type');
		//$article = dr('index/article.get_index', $type ? $type : '1,3', 3);
	...
	final public function postlogin()
	{
		...
		if ($u['password'] == $password) {
			if ($u['status'] == 1) {
				...
				redirect('demo.msg?key=login_username_email_activation');
			}

			if ($u['status'] == 0) {
				redirect('demo.msg?key=login_username_no_activation');
			}

			if ($u['status'] != 2) {
				redirect('demo.msg?key=login_username_lock');
			}
	...
	// array(11) { ["ajax_do"]=> string(1) "1" ["service_id"]=> string(1) "0" ["reg_platform"]=> string(5) "Win32" ["user_type"]=> string(10) "switch_web" ["username"]=> string(8) "nanrenwo" ["password"]=> string(6) "123456" ["password_confirm"]=> string(6) "123456" ["company_name"]=> string(0) "" ["company_domain"]=> string(0) "" ["qq"]=> string(4) "3213" ["phone"]=> string(11) "13111111111" }
	// array(11) { ["user_type"]=> string(13) "switch_advert" ["username"]=> string(8) "nanrenwo" ["password"]=> string(6) "123456" ["password_confirm"]=> string(6) "123456" ["company_name"]=> string(5) "qwewe" ["company_domain"]=> string(6) "wqeqew" ["qq"]=> string(4) "3213" ["phone"]=> string(11) "13111111111" }
	...
	{	
		...
		if ($this->is_valid_username($username)) {
			redirect('demo.msg?key=register_is_valid');
		}

		if (($username == NULL) || ($password == NULL) || (post('type') == 0)) {
			redirect('demo.msg?key=u_p_null');
		}

		if ((post('type') == 1) && ($GLOBALS['C_ZYIIS']['opne_affiliate_register'] != '1')) {
			redirect('demo.msg?key=close_affiliate_register');
		}

		if ((post('type') == 2) && ($GLOBALS['C_ZYIIS']['opne_advertiser_register'] != '1')) {
			redirect('demo.msg?key=close_advertiser_register');
		}
		...
		if ($u) {
			redirect('demo.msg?key=username_repeat_register');
		}
		...
		if ($GLOBALS['C_ZYIIS']['register_status'] == '2') {
			...
			$activateurl = 'http://' . $GLOBALS['C_ZYIIS']['authorized_url'] . url('demo.activate_users?u=' . $username . '&id=' . $activateid);
		...
		if ($GLOBALS['C_ZYIIS']['register_status'] == '2') {
			...
			redirect('demo.msg?key=register_email_activation');
		}
		else {
			redirect('demo.msg?key=success');
		}
	}
