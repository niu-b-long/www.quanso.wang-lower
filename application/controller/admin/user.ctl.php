...
class user_ctl extends admin_ctl
{
	...
	final public function update_ping()
	{
		// echo 'ok';
		if (is_post()) {
			$uid = post('uid');
			$pingid = post('pingid ');
			dr('admin/user.update_ping', $uid, $pingid);
			echo 'ok';
		}
	}

	final public function update_class()
	{
		// echo 'ok';
		$uid = post('uid');
		$classid = post('classid ');
		dr('admin/user.update_class', $uid, $classid);
		echo 'ok';
	}
	...
	final public function update_money()
	{
			...
			dr('admin/user.update_' . $monetype, $uid, $money);
          	$group = dr('admin/group.get_one',$user['groupid']);
			if($group != ''){
				dr('admin/pay.add_post',$uid,$money,$group['groupname']);
			}else{
            	if ($olb_money < $money) {
				$status = 3;
			}
